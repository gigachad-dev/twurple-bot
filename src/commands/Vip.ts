...
import type { HelixUser, HelixUserRelation } from '@twurple/api'
...
export default class Vip extends BaseCommand {
  ...
  private async claimVip({ channel, claimed }: PayloadVip): Promise<void> {
    ...
    if (vips.length === MAX_VIPS) {
      ...
      await this.client.api.chat.sendAnnouncement(channel.id, channel.id, {
  ...
  private async unVip(
    channel: HelixUser,
    user: HelixUser | HelixUserRelation
  ): Promise<void> {
  ...
  private async getVips(channel: HelixUser): Promise<HelixUserRelation[]> {
    const vips = await this.client.api.channels.getVips(channel.id, {
      limit: 100
    })
  ...
  private async checkForVips(
    channel: HelixUser,
    userId: string
  ): Promise<boolean> {
    return await this.client.api.channels.checkVipForUser(channel.id, userId)
  }
